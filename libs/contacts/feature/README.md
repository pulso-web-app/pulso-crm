...
Import contacts opens a dedicated dialog immediately before New contact. It accepts pasted JSON or reads a local `.json` file into the same editable source, validates automatically, reports item and field errors, and copies either a complete example or a Portuguese AI prompt without replacing the draft. Valid batches contain one to 500 contacts and commit atomically. Pending writes protect the dialog from edits and dismissal; failure retains the source for retry. Success reloads the first page, matching count, and global metrics from the server using the current filters and page size.
